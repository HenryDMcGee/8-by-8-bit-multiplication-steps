...
void addbinary(int c, int* acc, int* multiplicand) {
	//for loop for adding 8 bit binary
	for(int i = 0; i < 8; i++) {
		//when adding 1 and 0 without a carry, answer is 0
		if(acc[i] == 0 && multiplicand[i] == 1)  {
			if(c == 0)
				acc[i] = 1;
		}
		if(acc[i] == 1 && multiplicand[i] == 0) {
			if(c == 0)
				acc[i] = 1;
		}
		//when adding 1 and 0 with carry, answer is 0 with carry 
		else if(acc[i] == 0 && multiplicand[i] == 1)  {
			if(c == 1)
				acc[i] = 0;
		}
		else if(acc[i] == 1 && multiplicand[i] == 0) {
			if(c == 1)
				acc[i] = 0;
		}
...
void shiftRight(int* acc, int* mdr) {	
